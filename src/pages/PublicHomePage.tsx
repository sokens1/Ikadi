...
const PublicHomePage = () => {
  ...
  const fetchPublicResults = async () => {
    try {
      ...
      // Progression et participation basées sur l'élection courante
      const [{ count: pvValidatedCount }, { count: totalBureauxForElection }] = await Promise.all([
        supabase
          .from('procès_verbaux')
      ...
      const participation = (totalBureauxForElection || 0) > 0
        ? Math.min(((pvValidatedCount || 0) / (totalBureauxForElection || 0)) * 100, 100)
        : 0;
      const resultsProgress = participation;

      // Résultats candidats via la vue agrégée election_results_summary, fallback sur jointure si indisponible
      ...
      if (summaryRows && summaryRows.length > 0 && !summaryErr) {
        const totalVotes = summaryRows.reduce((sum: number, row: any) => sum + (Number(row.total_votes) || 0), 0);
        processed = summaryRows.map((row: any, index: number) => ({
          id: row.candidate_id,
          name: row.candidate_name,
          party: row.candidate_party || 'Indépendant',
          votes: Number(row.total_votes) || 0,
          percentage: totalVotes > 0 ? ((Number(row.total_votes) || 0) / totalVotes) * 100 : 0,
